import { Controller, Get, Post, Body, Param, Query, UsePipes, ParseUUIDPipe } from '@nestjs/common';
...
export class FeatureProposalController {
  ...
  @ApiQuery({ name: 'sortBy', required: false, enum: ['createdAt', 'upvoteCount'], example: 'createdAt' })
  @ApiQuery({ name: 'sortOrder', required: false, enum: ['asc', 'desc'], example: 'desc' })
  @ApiResponse({ status: 200, description: 'Paginated list of feature proposals', type: PaginatedFeatureProposalsResponseDto })
  async list(@Query(new ZodValidationPipe(ListFeatureProposalsSchema)) query: ListFeatureProposalsDto) {
    return this.listFeatureProposals.execute(query);
  }

  @Post()
  @ApiOperation({ summary: 'Create a new feature proposal' })
  @ApiResponse({ status: 201, description: 'Feature proposal created', type: FeatureProposalResponseDto })
  @ApiResponse({ status: 400, description: 'Validation error' })
  async create(
    @Body(new ZodValidationPipe(CreateFeatureProposalSchema)) body: CreateFeatureProposalBody,
  ) {
  ...
  @ApiResponse({ status: 201, description: 'Upvote registered', type: FeatureProposalResponseDto })
  @ApiResponse({ status: 404, description: 'Feature proposal not found' })
  @ApiResponse({ status: 409, description: 'Already upvoted' })
  async upvote(
    @Param('id', ParseUUIDPipe) id: string,
    @Body(new ZodValidationPipe(UpvoteFeatureProposalSchema)) body: UpvoteFeatureProposalBody,
  ) {
